...
class SortScreenVM with ChangeNotifier implements ISortScreenVM {
  ...
  @override
  List<SortOption> get sortOptions => _createSortOptions();

  @override
  List<FilterOption> get filterOptions => _createFilterOptions();

  @override
  SortOption? get selectedSort => _selectedSort;

  @override
  List<FilterOption> get selectedFilters => filterOptions.where((filter) => filter.isSelected).toList();
  ...
  void applySortingAndFilters(BuildContext context) {
    onApply(_selectedSort, _selectedFilterTypes);
  ...
  List<FilterOption> _createFilterOptions() {
    return [
      FilterOption(
        ...
        isSelected: _selectedFilterTypes.contains(FilterType.breakfast),
      ),
      FilterOption(
        title: 'Тренировка',
        description: '50–100 ккал, ≥12г углеводов, ≤0.3г жиров',
        type: FilterType.training,
        isSelected: _selectedFilterTypes.contains(FilterType.training),
      ),
      FilterOption(
        title: 'Сытость',
        description: '50–90 ккал, 10–15г углеводов, ≤10г сахара, ≥0.5г белка',
        type: FilterType.satiety,
        isSelected: _selectedFilterTypes.contains(FilterType.satiety),
      ),
      FilterOption(
        title: 'Перекус',
        description: '≤50 ккал, ≤7г сахара, ≤0.4г жиров',
        type: FilterType.snack,
        isSelected: _selectedFilterTypes.contains(FilterType.snack),
      ),
      FilterOption(
        title: 'Диета',
        description: '≤40 ккал, ≤6г сахара, ≤0.3г жиров',
        type: FilterType.diet,
        isSelected: _selectedFilterTypes.contains(FilterType.diet),
      ),
    ];
  }
}